<template>
  <header
    class="flex justify-between items-center w-full md:h-24 h-14 px-4 border-b-4 dark:border-white dark:border-opacity-5 bg-white bg-opacity-5"
  >
    <!-- Logo -->
    <div class="flex justify-start items-center">
      <div class="flex flex-shrink-0 md:w-14 w-8 md:h-14 h-8 rounded-full overflow-hidden">
        <img
          class="img"
          src="https://cdn.jsdelivr.net/gh/zhixinpeng/storage/img/avatar.png"
          :alt="name"
          width="56"
          height="56"
        />
      </div>
      <span class="md:ml-4 ml-2 md:text-2xl text-xl">{{ name }}</span>
    </div>

    <!-- 导航 - 移动端 -->
    <nav v-if="isMobile" class="nav flex flex-1 justify-end items-center text-xl">
      <a class="select-none mr-4" @click="toggleMenu">
        <ri-menu-fill v-show="!isShowMenu"></ri-menu-fill>
        <ri-close-fill v-show="isShowMenu"></ri-close-fill>
      </a>
      <Theme></Theme>
      <ul
        class="absolute top-14 left-0 flex flex-wrap w-full bg-gray-50 dark:bg-black border-b-4 dark:border-white dark:border-opacity-5 box-border px-4 z-10"
        v-if="isShowMenu"
      >
        <li
          v-for="(nav, index) in navList"
          :key="index"
          class="flex items-center w-1/3 h-14 text-base"
        >
          <router-link :to="nav.path" :title="nav.text" exact>{{ nav.text }}</router-link>
        </li>
      </ul>
    </nav>

    <!-- 导航 - 桌面端 -->
    <nav v-else class="flex flex-1">
      <ul class="flex justify-end items-center w-full">
        <li v-for="(nav, index) in navList" :key="index" class="mr-8 text-xl">
          <router-link :to="nav.path" :title="nav.text" exact>{{ nav.text }}</router-link>
        </li>
        <li>
          <a
            class="select-none mr-4 text-xl"
            href="https://github.com/zhixinpeng"
            target="_blank"
            rel="noopener noreferrer"
          >
            <ri-github-fill />
          </a>
        </li>
        <li>
          <Theme></Theme>
        </li>
      </ul>
    </nav>
  </header>
</template>

<script lang="ts" setup>
import { ref } from 'vue'
import { useRouter } from 'vue-router'
import isMobile from '@/shared/isMobile'
import config from '@/shared/config'

interface Nav {
  path: string
  text: string
}

const name = ref<string>(config.name)

// 导航菜单
const navList = ref<Nav[]>([])

navList.value = [
  {
    path: '/',
    text: 'Home'
  },
  {
    path: '/article',
    text: 'Article'
  },
  {
    path: '/about',
    text: 'About'
  }
]

// 移动端菜单开关
const isShowMenu = ref<boolean>(false)
const toggleMenu = () => {
  isShowMenu.value = !isShowMenu.value
}

// 切换路由后关闭菜单
const router = useRouter()
router.afterEach(() => {
  isShowMenu.value = false
})
</script>

<style lang="postcss" scoped="">
.nav a {
  display: flex;
  align-items: center;
  opacity: 0.7;
  text-decoration: none;
  transition: opacity 0.2s ease;

  svg {
    margin-right: calc(var(--margin) / 4);
  }

  &.router-link-active,
  &:hover {
    opacity: 1;
    text-decoration-color: inherit;
  }
}
</style>
